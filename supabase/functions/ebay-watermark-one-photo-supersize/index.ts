...
async function uploadSupersizeToEps(accessToken: string, imageUrl: string) {
  const xml = `<?xml version="1.0" encoding="utf-8"?>
<UploadSiteHostedPicturesRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  const text = await tradingCall(accessToken, "UploadSiteHostedPictures", xml)
  const ack = text.match(/<Ack>(.*?)<\/Ack>/)?.[1] ?? "Failure"
  if (ack !== "Success" && ack !== "Warning") {
    throw new Error(`UploadSiteHostedPictures failed: ${ebayErrorMessage(text)}`)
  }

  const fullUrl = decodeXml(
    text.match(/<FullURL>([\s\S]*?)<\/FullURL>/)?.[1] ?? "",
  )
  const pictureSet = decodeXml(
    text.match(/<PictureSet>([\s\S]*?)<\/PictureSet>/)?.[1] ?? "",
  )

  if (!fullUrl) {
    throw new Error("UploadSiteHostedPictures did not return FullURL")
  }

  return { fullUrl, pictureSet, ack }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const storagePath =
      `ebay-watermarked-supersize/${itemId}/${Date.now()}-${photoIndex + 1}.jpg`
    ...
    const eps = await uploadSupersizeToEps(accessToken, storageUrl)

    const revisedUrls = [...currentPictureUrls]
    revisedUrls[photoIndex] = eps.fullUrl
    ...
    return Response.json(
      {
        ...
        sourceUrl,
        storageUrl,
        epsFullUrl: eps.fullUrl,
        epsPictureSet: eps.pictureSet,
        uploadAck: eps.ack,
        ebayAck,
        jpegQuality: 95,
        message:
          `Photo ${photoIndex + 1} of ${currentPictureUrls.length} was rebuilt at JPEG quality 95 and uploaded to EPS as Supersize before revising the live listing.`,
      },
      { headers: corsHeaders },
    )
  } catch (error) {
