...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!sourceResponse.ok) {
      throw new Error(
        `Unable to download eBay photo ${photoIndex + 1} HTTP ${sourceResponse.status}`,
      )
    }
    ...
    const storagePath =
      `ebay-watermarked/${itemId}/${Date.now()}-${photoIndex + 1}.jpg`
    ...
    const epsUrl = await createEpsImageFromUrl(accessToken, storageUrl)

    const revisedUrls = [...pictureUrls]
    revisedUrls[photoIndex] = epsUrl
    ...
    return Response.json(
      {
        ...
        watermarkedUrl: epsUrl,
        ebayAck,
        message:
          `Photo ${photoIndex + 1} of ${pictureUrls.length} was watermarked and the live listing was revised while preserving all other photo positions.`,
      },
      { headers: corsHeaders },
    )
  } catch (error) {
